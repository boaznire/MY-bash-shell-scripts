...
FILESIZE1=$(du /tmp/doublecheck.txt | awk '{print $1}')
FILESIZE2=$(du /tmp//upgrade1.txt | awk '{print $1}')

if [ -s /tmp/upgrade1.txt ];then#In case that any error will occure and prevent the packge/s to be installed the script to modify 
#If the packges installed successfully we will receive email with the list of packges installed and how many
if [ $FILESIZE1 -gt 4 -a $FILESIZE2 -gt 0 ];then
...
if [ $FILESIZE2 -eq 0 ];then
 ...
 echo "No updates" | mail -s "Your Ubuntu server has been updated" username@domain.com
else 
sudo echo "Total updates: $count" >> /tmp/upgrade1.txt;
cat /tmp/upgrade1.txt | mail -s "Your ubuntu server has been updated" username@domain.com
fi

count1=$(wc -l /tmp/upgrade1.txt | awk '{print $1}')
echo
fi
